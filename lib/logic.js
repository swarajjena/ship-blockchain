  ...
  async function activateTask(assgn_reg,assgn,next_task,vessel,container){
      ...
      let TaskAllotment = getFactory().newEvent(ns, 'TaskAllotmentEvent');
      TaskAllotment.task = sresult[0];
      TaskAllotment.assigneeID=task.assigneeID;
      emit(TaskAllotment);      
  ...
  async function markTaskComplete(tParam,vParam,cParam,aParam) {
      ...
      if(sresult.length>0){
          ...
          let transactionNotification = getFactory().newEvent(ns, 'TransactionNotification');
          transactionNotification.task = sresult[0];
  ...
  async function dummy_fcc(tx) {
   ...
   sresult=await query('getTasksForParticipant',{assigneeParam:tx.getIdentifier()});
